...
# -*- coding: utf-8 -*-
# ============================================================
# Figure 3 (EN, v7): Model performance
#   ROC / Calibration / DCA — 统一共同模型（5 变量）
#   开发拟合 → 固定系数外部验证
# ============================================================
...
roc_df <- bind_rows(
  roc_to_df(roc_dev_m, "Development model (AUC 0.712)"),
  roc_to_df(roc_dev_g, "Development GNRI (AUC 0.651)"),
  roc_to_df(roc_dev_n, "Development NLR (AUC 0.675)"),
  roc_to_df(roc_val_m, "Validation, fixed coefficients (AUC 0.726)"))
roc_df$series <- factor(roc_df$series, levels = c(
  "Development model (AUC 0.712)", "Development GNRI (AUC 0.651)",
  "Development NLR (AUC 0.675)", "Validation, fixed coefficients (AUC 0.726)"))
...
cal_df <- bind_rows(
  transform(calib(comp_pub$icu, p_dev), cohort = "Development cohort"),
  transform(calib(comp_loc$case_icu, p_val_fixed), cohort = "Validation cohort"))
...
dca_df <- bind_rows(
  transform(dca(comp_pub$icu, p_dev, pt_seq), cohort = "Development model"),
  transform(dca(comp_loc$case_icu, p_val_fixed, pt_seq), cohort = "Validation model"))
...
W3 <- 190; H3 <- 62
pdf(file.path(OUT, "Figure3_ModelPerformance.pdf"), width = W3 / 25.4, height = H3 / 25.4)
print(fig3)
dev.off()
png(file.path(OUT, "Figure3_ModelPerformance.png"), width = W3, height = H3, units = "mm", res = 300)
print(fig3)
dev.off()

cat("Figure 3 (EN v7) done\n")
cat(sprintf("开发模型 AUC=%.3f | 验证固定系数 AUC=%.3f\n", auc(roc_dev_m), auc(roc_val_m)))

# ════════════════════════════════════════════════════════════
# Figure 4 (EN): 分层剂量反应（验证队列，独立于模型）
# ════════════════════════════════════════════════════════════
g_grp <- cut(comp_loc$gnri, breaks = c(-Inf, 82, 92, 98, Inf), right = FALSE,
             labels = c("Severe (<82)", "Moderate (82-91)", "Mild (92-97)", "Normal (≥98)"))
n_grp <- ifelse(comp_loc$nlr >= 2.96, "High NLR", "Low NLR")
...
g_df$strata <- factor(g_df$strata,
  levels = c("Severe (<82)", "Moderate (82-91)", "Mild (92-97)", "Normal (≥98)"))
n_df <- data.frame(strata = names(n_rate), rate = as.numeric(n_rate))
n_df$strata <- factor(n_df$strata, levels = c("High NLR", "Low NLR"))
...
p_g <- ggplot(g_df, aes(strata, rate, fill = strata)) +
  ...
  labs(x = NULL, y = "ICU transfer rate (%)") +
...
W4 <- 150; H4 <- 68
pdf(file.path(OUT, "Figure4_StratifiedRates.pdf"), width = W4 / 25.4, height = H4 / 25.4)
print(fig4)
dev.off()
png(file.path(OUT, "Figure4_StratifiedRates.png"), width = W4, height = H4, units = "mm", res = 300)
print(fig4)
dev.off()
cat("Figure 4 (EN) done\n")
